            ...
            $sql->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_WARNING);
                ...
                if($count === 1) {
                    foreach($response as $data) {
                        $responseData = json_encode(array("data"=>$data));
                    }
                    //forward the data 
                    ...
                    echo json_encode(array("invalid account" => "account not found" ));
                }      
        }
    }
class class_be_check_account_details {
            function func_be_check_account_details($accountnumber,$callback){
                $connection = new connection();
                $statement = 'SELECT * FROM  profile WHERE account_number = :accountnumber';
                $params  = array(':accountnumber' => $accountnumber);
